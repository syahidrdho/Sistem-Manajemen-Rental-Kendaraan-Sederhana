...
?>
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        ...
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-white">Edit Data Pelanggan</h2>
            <p class="text-text-secondary-dark mt-1">Perbarui detail pelanggan di bawah ini.</p>
        </div>

        <div class="bg-black/30 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-2xl border border-white/10 max-w-2xl mx-auto">
            
            <form action="index.php?page=pelanggan&action=edit&id=<?= htmlspecialchars($data['id_pelanggan']) ?>" method="POST" class="space-y-6">
                <input type="hidden" name="csrf_token" value="<?= CSRF::getToken() ?>">

                <div>
                    <label for="nama" class="block mb-2 text-sm font-medium text-text-primary-dark">Nama Lengkap</label>
                    <input 
                        type="text" 
                        id="nama" 
                        name="nama" 
