...
?>
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        ...
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-3xl font-bold text-white">Recycle Bin - Pelanggan</h2>
            <a href="index.php?page=pelanggan" class="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-primary rounded-lg shadow-lg shadow-primary/30 hover:bg-primary/90 transition-all duration-300">
                <span class="material-symbols-outlined text-base transform rotate-180">arrow_forward</span>
                Kembali ke Daftar
            </a>
        </div>

        <form method="POST" action="index.php?page=pelanggan&action=bulkRecycleBin">
                    ...
                    <table class="w-full text-left text-sm">
                        <thead class="text-xs text-text-secondary-dark uppercase border-b border-white/10">
                            <tr>
                                ...
                                <th class="px-6 py-4 font-semibold" scope="col">ID</th>
                                <th class="px-6 py-4 font-semibold" scope="col">Nama</th>
                                <th class="px-6 py-4 font-semibold" scope="col">Alamat</th>
                                <th class="px-6 py-4 font-semibold" scope="col">No. HP</th>
                        ...
                        <tbody class="text-text-primary-dark">
                            ...
                            <tr class="border-b border-white/10 hover:bg-white/5 transition-colors">
                                <td class="px-6 py-5">
                                    <input type="checkbox" name="ids[]" value="<?= $row['id_pelanggan'] ?>" class="rowCheckbox w-4 h-4 bg-white/10 border-white/30 rounded text-primary focus:ring-primary focus:ring-2">
                                </td>
                                <td class="px-6 py-5 font-medium">#<?= htmlspecialchars($row['id_pelanggan']) ?></td>
                                <td class="px-6 py-5"><?= htmlspecialchars($row['nama']) ?></td>
                                <td class="px-6 py-5"><?= htmlspecialchars($row['alamat']) ?></td>
                                <td class="px-6 py-5"><?= htmlspecialchars($row['no_hp']) ?></td>
                                <td class="px-6 py-5"><?= DateHelper::indoFull($row['deleted_at']) ?></td>
                                <td class="px-6 py-5 flex items-center gap-2">
                                    <a href="index.php?page=pelanggan&action=restore&id=<?= $row['id_pelanggan'] ?>" class="px-3 py-1 text-xs font-medium text-emerald-300 bg-emerald-500/20 rounded-md hover:bg-emerald-500/30 transition" onclick="return confirm('Anda yakin ingin mengembalikan data ini?')">Restore</a>
                                    <a href="index.php?page=pelanggan&action=deletePermanent&id=<?= $row['id_pelanggan'] ?>" class="px-3 py-1 text-xs font-medium text-red-300 bg-red-500/20 rounded-md hover:bg-red-500/30 transition" onclick="return confirm('ANDA YAKIN? Data ini akan hilang selamanya!')">Hapus Permanen</a>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        </tbody>
